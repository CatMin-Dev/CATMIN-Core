...
function initSidebar() {
  ...
  const currentPageParam = currentLocation.searchParams.get('page');
  ...
  if (currentLocation) {
    DOM.selectAll('#sidebar-menu a').forEach(link => {
      ...
      try {
        ...
        if (currentPageParam) {
          isCurrentLink = linkPageParam === currentPageParam;
        } else {
          isCurrentLink = linkUrl.pathname.replace(/\/+$/, '') === currentPath;
        }
      } catch (error) {
        isCurrentLink = false;
      }
