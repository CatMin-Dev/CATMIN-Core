    ...
    <form method="post" action="{{ admin_route('articles.store') }}" id="article-form">
        ...
        <div class="tab-content border border-top-0 rounded-bottom p-4 bg-white mb-4">
                ...
                <div class="row g-3">
                    <div class="col-12 col-lg-4">
                        ...
                        <select id="status" name="status" class="form-select @error('status') is-invalid @enderror">
                            ...
                            <option value="scheduled" @selected(old('status') === 'scheduled')>Programme</option>
            ...
            <div class="tab-pane fade" id="tab-taxonomy" role="tabpanel">
                <div class="row g-3">
                    @php($selectedTagIds = collect(old('tag_ids', []))->map(fn ($id) => (int) $id)->all())
                    <div class="col-12 col-lg-6">
                        ...
                        <select id="article_category_id" name="article_category_id" class="form-select @error('article_category_id') is-invalid @enderror">
                            ...
                            @foreach(($categories ?? collect()) as $category)
                                <option value="{{ $category['id'] }}" @selected((string) old('article_category_id') === (string) $category['id'])>{{ $category['name'] }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <select id="tag_ids" name="tag_ids[]" class="form-select @error('tag_ids') is-invalid @enderror" multiple size="8">
                            @foreach(($tags ?? collect()) as $tag)
                                <option value="{{ $tag->id }}" @selected(in_array((int) $tag->id, $selectedTagIds, true))>{{ $tag->name }}</option>
                            @endforeach
                        </select>
                        @error('tag_ids')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        <div class="form-text">Maintenez Ctrl/Cmd pour sélectionner plusieurs tags. Gestion complète via les écrans Tags/Catégories.</div>
                    </div>
                </div>
            </div>
