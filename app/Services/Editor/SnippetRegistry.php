...
class SnippetRegistry
{
    ...
    public function forContext(array $context = [], array $allowedIds = ['*']): array
    {
        ...
        foreach ((array) config('catmin_editor.snippet_registry', []) as $snippet) {
            if (!$this->isAllowed($snippet, $allowedIds, $context)) {
                continue;
            }
            $items[] = $this->normalize($snippet, 'snippet');
        }
        ...
        foreach (self::$providers as $provider) {
            ...
            foreach ((array) $provided as $snippet) {
                if (!$this->isAllowed($snippet, $allowedIds, $context)) {
                    continue;
                }
                $items[] = $this->normalize($snippet, 'snippet');
